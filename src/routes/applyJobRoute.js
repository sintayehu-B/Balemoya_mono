  ...
  async (req, res) => {
    try {
      ...
      const newPost = new JobApplied({
        ...req.body,
        applyUser: req.user.id,
        jobPost: req.params.id,
      });
      await addApplyForJobToUser(req.params.id, newPost.id);
...
router.get("/:id", user_auth, role_auth([roles.EMPLOYER]), async (req, res) => {
  try {
...
router.get("/", user_auth, role_auth([roles.EMPLOYER]), async (req, res) => {
  try {
    const post = await JobApplied.find()
      .populate("jobPost")
      .populate("applyUser")
      .exec();
    res.status(200).json(post);
