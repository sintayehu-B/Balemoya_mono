...
/**
 * Find the user by id and update the user's jobApplied array by pushing the jobApplied object into the
 * array.
 * @param user_id - the id of the user
 * @param jobApplied - {
 * @returns The user object with the jobApplied array updated.
 */
const addApplyForJobToUser = async (job_id, jobApplied) => {
  const jobpost = await JobPost.findById(job_id).populate("postedBy").exec();
  const { postedBy } = jobpost._doc;
  return await User.findByIdAndUpdate(
    postedBy.id,
    {
      $push: { jobApplied: jobApplied },
    },
    { new: true }
  ).exec();
};
...
/**
 * This function takes a user_id and a jobsPosted_id and adds the jobsPosted_id to the jobsPosted array
 * of the user with the user_id.
 * @param user_id - the id of the user who posted the job
 * @param jobsPosted_id - the id of the job post that was just created
 * @returns The updated user object.
 */
const addJobPostToUser = async (user_id, jobsPosted_id) => {
  return await User.findByIdAndUpdate(
    user_id,
    {
      $push: { jobsPosted: jobsPosted_id },
    },
    { new: true }
  ).exec();
};
